...
| 정책 추천 | 우선순위 top3 정책 카드 (자격조건/신청링크/예상혜택 포함) |
| 왜 추천했는지 | LLM 생성 설명문 (SHAP 요인 기반 자연어) |
| 히스토리 | 과거 진단 결과 변화 추이 (재방문 시) |
| 설정 | 알림 동의, 데이터 삭제 요청 |
...
| 로컬 캐시 | Room (선택, 오프라인 히스토리용) | |
| 차트 | Vico 또는 MPAndroidChart | 레이더차트 지원 확인 필요 |
| 인증 | 익명 세션ID (해커톤 데모용으로 충분) | |
...
| 정책 추천 | `GET /api/v1/citizen/{session_id}/recommendations` |
| 설명 문구 | `GET /api/v1/citizen/{session_id}/explanation` |
| 히스토리 | `GET /api/v1/citizen/{session_id}/history` |
