...
def _get_jwt_secret() -> str:
    """JWT_SECRET_KEY 환경변수를 우선 사용하고, 없으면 프로세스 수명 동안 유지되는
    임시 시크릿을 생성한다(재시작 시 기존 토큰은 모두 무효화됨 - 데모 단계에서는
    무방, 운영 전환 시 반드시 JWT_SECRET_KEY를 고정 값으로 설정할 것)."""
...
def create_access_token(user_id: str, expires_delta: timedelta | None = None) -> str:
    return _create_token(user_id, _TOKEN_TYPE_ACCESS, expires_delta or ACCESS_TOKEN_EXPIRE)


def create_refresh_token(user_id: str, expires_delta: timedelta | None = None) -> str:
    return _create_token(user_id, _TOKEN_TYPE_REFRESH, expires_delta or REFRESH_TOKEN_EXPIRE)
